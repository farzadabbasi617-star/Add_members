...
STAGGER_START = {
    "ultra": (0, 2),
    "safe": (0, 8),
    # نام‌های قدیمی، برای سازگاری با داده‌ی ذخیره‌شده در دیتابیس:
    # اگر عملیاتی با add_mode="fast" نیمه‌کاره مانده باشد، بعد از این
    # تغییر هم باید بتواند ادامه پیدا کند و KeyError ندهد.
    "max": (0, 2),
    "fast": (0, 8),
}
# بازه تاخیر انسانی بین هر ادد (ثانیه)
# سرعت را کاربر با انتخاب حالت تعیین می‌کند.
#
# ⚠️ نکته‌ای که دو بار به ما ضربه زد: این تأخیرِ «هر اکانت» است، ولی
# اکانت‌ها موازی کار می‌کنند. فاصله‌ی مؤثر روی گروه مقصد =
# میانگین_تأخیر ÷ تعداد_اکانت. با ۶ اکانت و safe=(45,95) یعنی یک ادد
# هر ~۱۲ ثانیه به یک گروه — که تلگرام آن را هجوم می‌بیند.
#
# ultra: فقط برای اکانت‌های قدیمی و گرم
# fast : تعادل
# safe : برای اکانت‌های نو یا وقتی مکرر PEER_FLOOD می‌گیریم
# فقط دو حالت به کاربر نشان داده می‌شود: «آهسته و امن» و «اولترا فست».
# نسخهٔ قبلی چهار حالت داشت (max/ultra/fast/safe) که تفاوتشان برای کاربر
# روشن نبود و عملاً کسی fast و max را از هم تشخیص نمی‌داد.
DELAY_RANGES = {
    "ultra": (8, 18),
    "safe": (60, 140),
    # نام‌های قدیمی — همان رفتار نزدیک‌ترین حالت جدید. حذفشان یعنی
    # KeyError روی هر عملیاتی که با نام قدیمی در دیتابیس ثبت شده.
    "max": (8, 18),
    "fast": (60, 140),
}
...
#: حالت‌هایی که در رابط کاربری نشان داده می‌شوند.
UI_ADD_MODES = ("safe", "ultra")
